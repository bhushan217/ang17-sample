...
import { ChunkLoadStrategy } from 'index-db/index-db-interfaces/api-base-response.interface';
...
export class ApiHandlerService implements IApiBaseActions {
  ...
  async GetAllChunks(url: string, chunkLoadStrategy: ChunkLoadStrategy) {
    ...
    if (totalRecords <= chunkLoadStrategy._limit) {
      return this.GetAll(url)?.toPromise();
    } else {
      let apiCalls: Observable<IApiBaseResponse>[] = [];
      let start = 0;
      const pageSize = chunkLoadStrategy._limit ?? this.PAGE_SIZE;
